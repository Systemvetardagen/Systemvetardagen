...
import { customFetchGetPreSignup } from "@/lib/utilities/customFetchGetPreSignUp";
import { user } from "@/lib/types/genstruct";
...
const StudentDashboard: React.FC = () => {
  ...
  const [loginState, setLoginState] = useState<"initial" | "fail" | "success">(
    "initial"
  );
  ...
  const { data: signUpData, isFetching: signUpsIsFetching } = useQuery({
    enabled:
      loginData.username != "" &&
      loginData.password != "" &&
      students.length === 0,
  ...
  useEffect(() => {
    if (signUpData) {
      ...
      setStudents((prev) =>
        JSON.stringify(prev) !== JSON.stringify(signUpData.signups)
          ? signUpData.signups
          : prev
      );
      setError(null);
  ...
  }, [signUpData, loginData.username, loginData.password]);
        ...
        <div className="flex justify-between items-center flex-wrap gap-4 mb-6">
          ...
          <div className="flex gap-4">
            <button
              id="pagetext"
              onClick={copyEmails}
              disabled={signUpsIsFetching || students.length === 0}
